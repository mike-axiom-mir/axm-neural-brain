...
## Executable growth laboratory

The [Neural Innovation Laboratory](INNOVATION_LAB.md) combines the existing core,
hybrid workspace, persistence and learning evidence with isolated descendant
experiments and a learned experiment chooser. Its receipt covers 36 held-out
synthetic task instances, preserves regressions and keeps adoption explicit.
It is not an open-ended invention or UC-runtime claim. See the laboratory guide
for commands, measurements, limitations and complete archive reconstruction.
...
The package initializer remains narrower than the donor initializer. The integrated
`axm_persistence` package owns Genesis/Continuity mechanics alongside the core;
the core keeps the canonical root contract required by snapshots.
